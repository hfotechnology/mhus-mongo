...
/** @morphia.internal this class will be internalized in 2.0 */
...
public class Mapper {
    ...
    /**
     * Special name that can never be used. Used as default for some fields to indicate default
     * state.
     *
     * @morphia.internal
     */
    ...
    /**
     * Special field used by morphia to support various possibly loading issues; will be replaced
     * when discriminators are implemented to support polymorphism
     */
    ...
    protected final Map<String, MappedClass> mappedClasses =
            new ConcurrentHashMap<String, MappedClass>();

    protected final ConcurrentHashMap<String, Set<MappedClass>> mappedClassesByCollection =
            new ConcurrentHashMap<String, Set<MappedClass>>();

    // EntityInterceptors; these are called after EntityListeners and lifecycle methods on an
    ...
    private final LazyProxyFactory proxyFactory =
            LazyFeatureDependencies.createDefaultProxyFactory();
    ...
    /**
     * Creates a new Mapper with the given options and a Mapper to copy. This is effectively a copy
     * constructor that allows a developer to override the options.
     *
     ...
     * @morphia.internal no replacement is planned
     */
    public <T> T fromDBObject(
        ...
        return fromDb(
                datastore,
                dbObject,
                opts.getObjectFactory().createInstance(entityClass, dbObject),
                cache);
    }
    ...
    public <T> T fromDb(
        ...
        if (dbObject.containsField("_id")
            ...
            final Key<T> key =
                    new Key(
                            entity.getClass(),
                            getCollectionName(entity.getClass()),
                            dbObject.get("_id"));
    ...
    public Class<?> getClassFromCollection(final String collection) {
        ...
        if (mcs == null || mcs.isEmpty()) {
            throw new MappingException(
                    format("The collection '%s' is not mapped to a java class.", collection));
        }
        if (mcs.size() > 1) {
            if (LOG.isInfoEnabled()) {
                LOG.info(
                        format(
                                "Found more than one class mapped to collection '%s'%s",
                                collection, mcs));
            }
        }
     ...
     *
     * @param entity the entity to process
     * @param collection the collection to use in the Key rather than the mapped collection as
     *     defined on the entity's class
     * @param <T> the type of the entity
     * @return the Key
     */
    ...
    /**
     * Gets the {@link MappedClass} for the object (type). If it isn't mapped, create a new class
     * and cache it (without validating).
     *
    ...
    public <T> Key<T> refToKey(final DBRef ref) {
        return ref == null
                ? null
                : new Key<T>(
                        (Class<? extends T>) getClassFromCollection(ref.getCollectionName()),
                        ref.getCollectionName(),
                        ref.getId());
    }

    /**
     * /** Converts an entity (POJO) to a DBObject. A special field will be added to keep track of
     * the class type.
     *
    ...
    /**
     * Converts a java object to a mongo-compatible object (possibly a DBObject for complex
     * mappings). Very similar to {@link Mapper#toDBObject}. Used (mainly) by query/update
     * operations.
     *
    ...
    public Object toMongoObject(final MappedField mf, final MappedClass mc, final Object value) {
        ...
        if (value instanceof Query) {
            ...
            // convert the value to Key (DBRef) if the field is @Reference or type is Key/DBRef, or
            // if the destination class is an @Entity
            try {
                if (value instanceof Iterable) {
                    MappedClass mapped = getMappedClass(mf.getSubClass());
                    if (mapped != null
                            && (Key.class.isAssignableFrom(mapped.getClazz())
                                    || mapped.getEntityAnnotation() != null)) {
                    ...
                    if (mf != null) {
                        ...
                        if (!mf.getType().equals(Key.class) && isMapped(mf.getType())) {
                            final MappedField idField =
                                    getMappedClass(mf.getType()).getMappedIdField();
                        ...
                        if (refAnn != null) {
                            ...
                            if (mappedValue == value) {
                                throw new ValidationException(
                                        "cannot map to Key<T> field: " + value);
                            }
                        }
                    }
            ...
            try {
                mappedValue =
                        Serializer.serialize(
                                value, !mf.getAnnotation(Serialized.class).disableCompression());
            } catch (IOException e) {
            ...
            mappedValue =
                    toMongoObject(
                            value, EmbeddedMapper.shouldSaveClassName(value, mappedValue, mf));
            if (mappedValue instanceof BasicDBList) {
                final BasicDBList list = (BasicDBList) mappedValue;
                if (list.size() != 0) {
                    if (!EmbeddedMapper.shouldSaveClassName(
                            extractFirstElement(value), list.get(0), mf)) {
                        for (Object o : list) {
    ...
    /**
     * Updates the @{@link dev.morphia.annotations.Id} and @{@link dev.morphia.annotations.Version}
     * fields.
     *
     * @param datastore the Datastore to use when fetching this reference
    ...
    public void updateKeyAndVersionInfo(
        ...
        if ((mc.getIdField() != null) && (dbObj != null) && (dbObj.get("_id") != null)) {
            try {
                ...
                if (oldIdValue != null) {
                    // The entity already had an id set. Check to make sure it hasn't changed. That
                    // would be unexpected, and could
                    ...
                    if (!dbIdValue.equals(oldIdValue)) {
                        ...
                        throw new RuntimeException(
                                format(
                                        "@Id mismatch: %s != %s for %s",
                                        oldIdValue, dbIdValue, entity.getClass().getName()));
                    }
                }
            } catch (Exception e) {
    ...
    protected Class<? extends Annotation> getFieldAnnotation(final MappedField mf) {
        ...
        for (final Class<? extends Annotation> testType :
                new Class[] {Property.class, Embedded.class, Serialized.class, Reference.class}) {
    ...
    <T> Key<T> manualRefToKey(final String collection, final Object id) {
        return id == null
                ? null
                : new Key<T>(
                        (Class<? extends T>) getClassFromCollection(collection), collection, id);
    }
    ...
    /**
     * Converts a java object to a mongo-compatible object (possibly a DBObject for complex
     * mappings). Very similar to {@link Mapper#toDBObject}
     *
     * <p>Used (mainly) by query/update operations
     */
            ...
            if (type.isArray()
                ...
                subType =
                        (type.isArray())
                                ...
                                : getParameterizedClass(type, (isMap) ? 1 : 0);
            }
