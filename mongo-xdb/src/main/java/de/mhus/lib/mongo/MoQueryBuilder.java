    ...
    private void createQuery(APrint p, StringBuilder s) {
        ...
        if (p instanceof AEnumFix) {
            ...
            s.append(",\"$offset\":\"")
                    ...
                    .append("\",\"$limit\":\"")
                    .append(((ALimit) p).getLimit())
                    .append("\""); // mysql specific !!
        } else
        ...
        if (p instanceof AOr) {
            ...
            // manager.toQualification(((ASubQuery)p).getSubQuery()).trim();
            //
            ...
            //
            //			buffer.append( manager.createSqlSelect(((ASubQuery)p).getSubQuery().getType(),
            // buffer2.toString() , qualification));
            //
            //			buffer.append(")");
            //		} else
    ...
    private <T> void add(Query<T> q, ObjectNode j, Map<String, Object> parameterValues)
        ...
        for (Iterator<Entry<String, JsonNode>> ki = j.fields(); ki.hasNext(); ) {
            ...
            if (k.equals("$or")) { // "$or":[{"name":"Max"},{"name":"Sabine"}]
                ...
                q.order(v.asText());
            } else if (k.equals("$where")) {
                q.where(v.asText());
            } else if (k.equals("$offset")) {
                q.offset(v.asInt());
            } else if (k.equals("$limit")) {
                q.limit(v.asInt());
            } else setValue(q.field(k), k, v, parameterValues);
        }
    }

    private <T> Object setValue(
            FieldEnd<?> f, String k, JsonNode v, Map<String, Object> parameterValues)
            throws IOException {
        if (v.isObject()) {
            ...
            switch (c) {
                case "$lt":
                    return f.lessThan(vv);
                case "$lte":
                    return f.lessThanOrEq(vv);
                case "$eq":
                    return f.equal(vv);
                case "$gt":
                    return f.greaterThan(vv);
                case "$gte":
                    return f.greaterThanOrEq(vv);
                case "$ne":
                    return f.notEqual(vv);
                case "$contains":
                    return f.contains(vv);
                case "$containsIgnoreCase":
                case "$text":
                    return f.containsIgnoreCase(vv);
                default:
                    throw new IOException("Unknown condition " + c);
            }
        } else {
