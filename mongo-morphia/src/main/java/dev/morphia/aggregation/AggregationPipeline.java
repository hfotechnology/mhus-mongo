...
public interface AggregationPipeline {
     ...
     * using the default options as defined in {@link AggregationOptions}.
     *
     * <p><b>Note: This return type will change to {@code MongoCursor} in 2.0 to allow for
     * finer-grained control of iteration and resource management.</b>
     *
     ...
     * finer-grained control of iteration and resource management.</b>
     *
     * @param collectionName The collection in which to store the results of the aggregation
     *     overriding the mapped value in target
     * @param target The class to use when iterating over the results
     * @param options The options to apply to this aggregation
     * @param readPreference The read preference to apply to this pipeline
     * @param <U> type of the results
     ...
     * Incorporates the functionality of $match, $sort, and $limit for geospatial data. The output
     * documents include an additional distance field and can include a location identifier field.
     *
     ...
     * document per each distinct group. The output documents only contain the identifier field and,
     * if specified, accumulated fields. The ID for this group is null.
     *
     ...
     * expression(s), if specified, to each group. Consumes all input documents and outputs one
     * document per each distinct group. The output documents only contain the identifier field and,
     * if specified, accumulated fields.
     *
     ...
     * documents from the "joined" collection for processing. The $lookup stage does an equality
     * match between a field from the input documents with a field from the documents of the
     * “joined” collection. To each input document, the $lookup stage adds a new array field whose
    ...
    /**
     * Places the output of the aggregation in the collection mapped by the target type using the
     * default options as defined in {@link AggregationOptions}.
     *
     * @param target The class to use when iterating over the results
     * @param <U> type of the results
     * @return an iterator of the computed results
     ...
     * documents unmodified to the pipeline. For each input document, outputs either zero documents
     ...
     *
     * <p>Each output document contains two fields: an _id field containing the distinct grouping
     * value, and a count field containing the number of documents belonging to that grouping or
     * category.
     *
     ...
     *
     * <p>Each bucket is represented as a document in the output. The document for each bucket
     * contains an _id field, whose value specifies the inclusive lower bound of the bucket and a
     * count field that contains the number of documents in the bucket. The count field is included
     ...
     *     boundaries for each bucket. Each adjacent pair of values acts as the inclusive lower
     *     boundary and the exclusive upper boundary for the bucket. You must specify at least two
     *     boundaries.
     * @return this
     ...
     *
     * <p>Each bucket is represented as a document in the output. The document for each bucket
     * contains an _id field, whose value specifies the inclusive lower bound and the exclusive
     * upper bound for the bucket, and a count field that contains the number of documents in the
