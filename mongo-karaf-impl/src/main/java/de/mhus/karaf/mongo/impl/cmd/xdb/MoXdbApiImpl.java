...
public class MoXdbApiImpl implements XdbApi {
    ...
    public XdbService getService(String serviceName) throws NotFoundException {
        for (MoManagerService service :
                new TreeSet<MoManagerService>(MongoUtil.getManagerServices())) {
            if (serviceName.equals(service.getServiceName())) return new Service(service);
        }
    ...
    public <T> XdbType<T> getType(String serviceName, String typeName) throws NotFoundException {
        ...
        for (MoManagerService service :
            ...
            if (serviceName.equals(service.getServiceName())) {
                ...
                if (type == null)
                    throw new NotFoundException("Type not found", name, service.getServiceName());
    ...
    private static class Service implements XdbService {
        ...
        public <T> XdbType<T> getType(Class<T> type) throws NotFoundException {
            ...
            if (out == null)
                throw new NotFoundException("Type not found", type, service.getServiceName());
    ...
    private static class Type<T> implements XdbType<T> {
        ...
        public List<String> getAttributeNames() {
            LinkedList<String> out = new LinkedList<>();
            for (PojoAttribute<?> f : model) out.add(f.getName());
            return out;
        }
        ...
        public T newInstance() throws Exception {
            return service.getManager().inject((T) type.getDeclaredConstructor().newInstance());
        }
