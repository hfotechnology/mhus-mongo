...
public final class EntityTypeAndIdValueValidator implements Validator {
    private static final EntityTypeAndIdValueValidator INSTANCE =
            new EntityTypeAndIdValueValidator();
    ...
    /**
     * Checks the class of the {@code value} against the type of the ID for the {@code type}. Always
     * applies this validation, but there's room to change this to not apply it if, for example, the
     ...
     *     appliesTo} returned false, this list will not change. @return true if the validation was
     *     applied.
     */
    public boolean apply(
        ...
        if (appliesTo(mappedClass, mappedField)) {
            ...
            if (!mappedField.getType().equals(classOfValue)
                    && !classOfValue.equals(classOfIdFieldForType)) {
                validationFailures.add(
                        new ValidationFailure(
                                format(
                                        "The value class needs to match the type of ID for the field. "
