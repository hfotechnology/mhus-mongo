...
public interface AdvancedDatastore extends Datastore {
     ...
     * @return Query for the specified class clazz this feature is being removed. no replacement is
     *     planned. see issue #1331
     */
    ...
    /**
     * Creates a reference to the entity (using the current DB -can be null-, the collectionName,
     * and id)
     *
    ...
    /**
     * Deletes an entity of the given type T, with the given {@code id}, from the collection with
     * the name in the {@code kind} param. Validates the {@code id}, checking it's the correct type
     * for an ID for entities of type {@code T}. The entity type {@code clazz} is used only for
     * validation, not for filtering, therefore if you have entities of different types in the same
     * collection ({@code kind}), this method will delete any entity with the given {@code id},
     ...
     * @morphia.inline use {@link #find(String, Class)} and {@link #delete(Query)} instead
     */
     ...
     * @return the result of this delete operation.
     * @morphia.inline
     * @since 1.3 use {@link #find(String, Class)} and {@link #delete(Query, DeleteOptions)} instead
     */
     ...
     * @param <T> the entity type
     * @param <V> is the type of the ID, for example ObjectId
     * @return the result of this delete operation. use {@link #find(String, Class)} and {@link
     *     #delete(Query, DeleteOptions)} instead
     */
     ...
     * @see #ensureIndexes(String, Class) This method uses the legacy approach for defining indexes.
     *     Switch to using annotations on entity classes or the methods in the Java driver itself.
     */
     ...
     * @param dropDupsOnCreate This value is no longer supported. Tells the unique index to drop
     *     duplicates silently when creating; only the first will be kept
     * @param <T> the type to index
     * @see MongoCollection#createIndex(org.bson.conversions.Bson,
     *     com.mongodb.client.model.IndexOptions)
     * @see #ensureIndexes(String, Class) This method uses the legacy approach for defining indexes.
     ...
     * {@code @Indexed, @Indexes)} on the given collection name.
     *
     * @param collection the collection to update
     * @param clazz the class from which to get the index definitions
     * @param <T> the type to index this feature is being removed. no replacement is planned. see
     *     issue #1331
     */
     ...
     * @param collection the collection to update
     * @param clazz the class from which to get the index definitions
     * @param background if true, the index will be built in the background. If false, the method
     *     will block until the index is created.
     * @param <T> the type to index use {@link #ensureIndexes(String, Class)} instead and specify
     *     background true or false in the annotations
     */
     ...
     * @param readPreference Uses the supplied ReadPreference for the check. If readPreference is
     *     null the preference is taken from the annotation or uses the default preference.
     * @return the key if the entity exists
     * @morphia.inline
     * @see #exists(Object) use {@link Query#first()} instead
     */
     ...
     * @return the query this feature is being removed. no replacement is planned. see issue #1331
     */
     ...
     * @return the query
     * @morphia.inline use {@link #find(String, Class)} and apply the options to the {@link
     *     com.mongodb.client.model.FindOptions} parameter
     */
    <T, V> Query<T> find(
            String collection, Class<T> clazz, String property, V value, int offset, int size);
     ...
     * @morphia.inline use {@link #find(Class)} instead
     */
     ...
     * @morphia.inline use {@link #find(String, Class)} instead
     */
     ...
     * @morphia.inline use {@link Query#count()} instead
     */
     ...
     * @return the new key of the inserted entity
     * @morphia.inline
     * @see WriteConcern use {@link #insert(Object, InsertOptions)}
     */
     ...
     * @param entity the entity to insert
     * @param <T> the type of the entity
     * @return the new key of the inserted entity this feature is being removed. no replacement is
     *     planned. see issue #1331
     */
     ...
     * @return the new key of the inserted entity
     * @morphia.inline
     * @since 1.3 this feature is being removed. no replacement is planned. see issue #1331
     */
     ...
     * @morphia.inline use {@link #insert(Iterable)} instead
     */
     ...
     * @param wc the WriteConcern to use when inserting
     * @param <T> the type of the entity
     * @return the new keys of the inserted entities use {@link #insert(Iterable, InsertOptions)}
     */
     ...
     * @param entities the entities to insert
     * @param <T> the type of the entity
     * @return the new keys of the inserted entities this feature is being removed. no replacement
     *     is planned. see issue #1331
     */
     ...
     * @param <T> the type of the entity
     * @return the new keys of the inserted entities
     * @see WriteConcern use {@link #insert(String, Iterable, InsertOptions)} instead
     */
     ...
     * @param wc the WriteConcern to use when inserting
     * @param <T> the type of the entity
     * @return the new key of the inserted entity use {@link #save(String, Object, InsertOptions)}
     *     instead
     */
