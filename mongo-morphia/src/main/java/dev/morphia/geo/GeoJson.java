 ...
 * .org/manual/applications/geospatial-indexes/#geojson-objects">the documentation</a> for all the
 * types.
 */
public final class GeoJson {
    ...
    /**
     * Create a new Point representing a GeoJSON point type. For a safer way to create points with
     * latitude and longitude coordinates without mixing up the order, {@link PointBuilder}.
     *
     * @param latitude the point's latitude coordinate
     * @param longitude the point's longitude coordinate
     * @return a Point instance representing a single location point defined by the given latitude
     *     and longitude
     * @mongodb.server.release 2.4
     * @see dev.morphia.geo.PointBuilder
     * @see <a
     *     href="http://docs.mongodb.org/manual/apps/geospatial-indexes/#geojson-objects">GeoJSON</a>
     */
    ...
    /**
     * Create a new Polygon representing a GeoJSON Polygon type. This helper method uses {@link
     * #polygon(LineString, LineString...)} to create the Polygon. If you need to create Polygons
     * with interior rings (holes), use that method.
     *
     * @param points an ordered series of Points that make up the polygon. The first and last points
     *     should be the same to close the polygon
    ...
    private static void ensurePolygonIsClosed(final LineString points) {
        ...
        if (size > 0
                && !points.getCoordinates().get(0).equals(points.getCoordinates().get(size - 1))) {
            throw new IllegalArgumentException(
                    "A polygon requires the starting point to be the same as the end to ensure a closed "
                            + "area");
        }
    }

    /**
     * Lets you create a Polygon representing a GeoJSON Polygon type. This method is especially
     * useful for defining polygons with inner rings.
     *
     * @param exteriorBoundary a LineString that contains a series of Points that make up the
     *     polygon. The first and last points should be the same to close the polygon
     * @param interiorBoundaries optional varargs that let you define the boundaries for any holes
     *     inside the polygon
    ...
    public static Polygon polygon(
            final LineString exteriorBoundary, final LineString... interiorBoundaries) {
        ensurePolygonIsClosed(exteriorBoundary);
